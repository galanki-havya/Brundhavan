...
export default function Footer() {
  ...
  return (
    <footer style={{ background: '#0d1117', color: '#e2e8f0', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ── ACCENT BAR ── */}
      <div style={{ height: '3px', background: 'linear-gradient(90deg, #C4391E, #FF6347, #f97316)' }} />
      ...
      <div style={{ background: '#111827', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '16px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          />
          <button style={{ background: '#C4391E', color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 18px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
            Subscribe
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr 1fr 1.3fr', gap: '40px', padding: '48px 40px 36px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
            <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'rgba(196,57,30,0.15)', border: '1px solid rgba(196,57,30,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <School size={18} style={{ color: '#C4391E' }} />
            </div>
            <div>
              <div style={{ fontSize: '17px', fontWeight: 700, color: '#fff', letterSpacing: '-0.01em' }}>{school.name}</div>
              <div style={{ fontSize: '10px', color: '#C4391E', letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 600 }}>{school.affiliation}</div>
            </div>
          </div>
          <p style={{ fontSize: '13px', color: '#64748b', lineHeight: 1.75, margin: '14px 0 20px', maxWidth: '240px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {socials.map(({ href, Icon, label }) => (
              <a key={label} href={href} target="_blank" rel="noreferrer" aria-label={label}
                ...
                onMouseEnter={e => { e.currentTarget.style.background = '#C4391E'; e.currentTarget.style.borderColor = '#C4391E'; e.currentTarget.style.color = '#fff' }}
        ...
        {/* QUICK LINKS */}
        <div>
          <h4 style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#C4391E', marginBottom: '18px' }}>Quick Links</h4>
          <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {quickLinks.map(link => (
              <li key={link.name}>
                <NavLink to={link.path}
                  style={({ isActive }) => ({ fontSize: '13px', color: isActive ? '#e2e8f0' : '#64748b', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px', transition: 'color 0.2s' })}
        ...
        {/* PROGRAMMES */}
        <div>
          <h4 style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#C4391E', marginBottom: '18px' }}>Programmes</h4>
          <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {programmes.map(link => (
              <li key={link.name}>
                <NavLink to={link.path}
        ...
        {/* CONTACT */}
        <div>
          <h4 style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#C4391E', marginBottom: '18px' }}>Contact Us</h4>
          {[
            { Icon: MapPin, content: school.address },
            { Icon: Phone, content: <a href={`tel:${school.phone}`} style={{ color: '#94a3b8', textDecoration: 'none' }}>{school.phone}</a> },
            { Icon: Mail,  content: <a href={`mailto:${school.email}`} style={{ color: '#94a3b8', textDecoration: 'none', wordBreak: 'break-all' }}>{school.email}</a> },
          ].map(({ Icon, content }, i) => (
            <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', marginBottom: '14px' }}>
              <div style={{ width: '32px', height: '32px', background: 'rgba(196,57,30,0.12)', border: '1px solid rgba(196,57,30,0.25)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Icon size={14} style={{ color: '#C4391E' }} />
              </div>
              <div style={{ fontSize: '12.5px', color: '#64748b', lineHeight: 1.6 }}>{content}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 40px', flexWrap: 'wrap', gap: '8px', maxWidth: '1200px', margin: '0 auto' }}>
        <span style={{ fontSize: '12px', color: '#374151' }}>© {new Date().getFullYear()} {school.name}. All rights reserved.</span>
        <div style={{ display: 'flex', gap: '20px' }}>
          {['Privacy Policy', 'Terms of Use', 'Sitemap'].map(t => (
            <a key={t} href="#" style={{ fontSize: '12px', color: '#374151', textDecoration: 'none' }}
