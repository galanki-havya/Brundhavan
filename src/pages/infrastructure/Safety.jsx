...
import { Shield, Eye, AlertTriangle, Heart, CheckCircle2, ArrowRight, Lock, Bell, Users, Activity } from 'lucide-react'
import PageHero from '../../components/PageHero'

const featureCards = [
  { emoji: '🛡️', label: 'AI-Assisted Security', desc: 'AI-powered surveillance with real-time monitoring and intelligent threat detection.', bg: '#FFF8F6', border: '#FFCFC4' },
  { emoji: '📷', label: 'CCTV Coverage',        desc: 'Comprehensive cameras covering classrooms, corridors, entry points, and playgrounds.', bg: '#EEF3FB', border: '#B8CFF0' },
  { emoji: '🚨', label: 'Emergency Response',   desc: 'Rapid response systems for medical and safety emergencies with trained staff.',       bg: '#FFFBEE', border: '#F5DFA0' },
  { emoji: '❤️', label: 'Health & Wellness',    desc: 'On-site medical facility, health monitoring, and a dedicated student counsellor.',    bg: '#F0FAF4', border: '#A8DDB8' },
]

const securityCols = [
  {
    title: 'Physical Security', icon: Lock,
  ...
  {
    title: 'Digital Security', icon: Eye,
  ...
  {
    title: 'Safety Education', icon: Users,
  ...
  return (
    <div style={{ background: '#F9FAFB', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ background: '#FFFFFF', padding: '80px 0' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }} className="safety-grid">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              <span style={{ color: '#FF6347', fontWeight: 600, fontSize: 11, letterSpacing: '3px', textTransform: 'uppercase' }}>Our Commitment</span>
            </div>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 700, color: '#011E3A', lineHeight: 1.2, marginBottom: '20px' }}>
              Your Child's Safety is Our{' '}<span style={{ color: '#FF6347', fontStyle: 'italic', fontWeight: 400 }}>Top Priority</span>
            </h2>
            ...
            {['AI-powered 24/7 surveillance across all campus areas', 'Gated entry with controlled access', 'Trained security personnel round the clock', 'On-site medical facility with qualified nurse', 'Real-time parent communication & emergency alerts'].map((point, i) => (
      ...
      <section style={{ padding: '80px 0', background: '#F9FAFB' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginBottom: '52px' }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', fontWeight: 700, color: '#011E3A', marginBottom: '10px' }}>Safety at a Glance</h2>
            <p style={{ color: '#302f2f', fontSize: '1.2rem' }}>Every system designed to protect what matters most</p>
          </motion.div>
      ...
      <section style={{ padding: '80px 0', background: '#FFFFFF' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginBottom: '52px' }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', fontWeight: 700, color: '#011E3A' }}>Security Infrastructure</h2>
          </motion.div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
            {securityCols.map((col, i) => (
