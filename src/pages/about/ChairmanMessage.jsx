    ...
    <div style={{ background: '#fff', fontFamily: "'Playfair Display', 'Georgia', serif" }}>
      ...
      <section style={{ padding: '80px 0', background: '#fff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '48px', alignItems: 'start' }}>
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} style={{ position: 'sticky', top: '32px' }}>
            <div style={{ background: '#fff', borderRadius: '24px', padding: '32px 24px', textAlign: 'center', border: '1.5px solid #FFD8CC', boxShadow: '0 8px 32px rgba(255,99,71,0.1)', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: 'linear-gradient(90deg, #FF6347, #FFBDAA)' }} />

              <div style={{ width: '160px', height: '160px', borderRadius: '20px', overflow: 'hidden', margin: '0 auto 20px', border: '3px solid #FFD8CC', boxShadow: '0 8px 24px rgba(255,99,71,0.15)' }}>
              ...
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', color: '#FF6347', fontWeight: 600, marginBottom: '20px', fontStyle: 'italic' }}>{chairman.title}</p>

              <div style={{ borderTop: '1px solid #FFD8CC', paddingTop: '20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', fontFamily: "'DM Sans', sans-serif", fontSize: '0.85rem', color: '#3d2c40' }}>
                    <item.icon size={16} style={{ color: '#FF6347', flexShrink: 0 }} />
                    <span>{item.text}</span>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '20px' }}>
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 + i * 0.1 }} style={{ background: '#fff', borderRadius: '14px', padding: '14px 18px', border: '1px solid #FFD8CC', boxShadow: '0 2px 12px rgba(255,99,71,0.06)', display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'linear-gradient(135deg, #FFD8CC, #FFF3F0)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '1px solid #FFBDAA' }}>
                    <item.icon size={18} style={{ color: '#FF6347' }} />
                  </div>
                  <div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
              <div style={{ width: '4px', height: '48px', background: 'linear-gradient(180deg, #FF6347, #FFBDAA)', borderRadius: '4px', flexShrink: 0 }} />
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: '2rem', fontWeight: 700, color: '#1a1a2e', margin: 0 }}>About the Chairman</h2>
            </div>

            {/* Quote accent */}
            <div style={{ background: '#fff', borderRadius: '16px', padding: '24px 28px', border: '1.5px solid #FFD8CC', marginBottom: '32px', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '3px', background: 'linear-gradient(90deg, #FF6347, #FFBDAA)' }} />
              <Quote size={32} style={{ color: '#FFBDAA', marginBottom: '12px' }} />
            ...
            {/* Signature */}
            <div style={{ borderTop: '1px solid #FFD8CC', paddingTop: '28px' }}>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.3rem', fontWeight: 700, color: '#1a1a2e', marginBottom: '4px' }}>{chairman.name}</p>
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.8rem', fontWeight: 600, color: '#FF6347', letterSpacing: '2px', textTransform: 'uppercase', margin: 0 }}>{chairman.title}</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 24px', background: '#fff', textAlign: 'center' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <div style={{ width: '60px', height: '3px', background: 'linear-gradient(90deg, #FF6347, #FFBDAA)', borderRadius: '4px', margin: '0 auto 24px' }} />
      ...
      <section style={{ padding: '80px 24px', background: 'linear-gradient(135deg, #C4391E 0%, #FF6347 50%, #FF8C6B 100%)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '680px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/admissions" style={{ background: '#fff', color: '#C4391E', fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: '0.97rem', padding: '14px 32px', borderRadius: '50px', textDecoration: 'none', boxShadow: '0 8px 30px rgba(0,0,0,0.15)', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>Start Your Child's Journey <ArrowRight size={17} /></Link>
