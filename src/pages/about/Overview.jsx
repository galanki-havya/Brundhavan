  ...
  return (
    <div style={{background: '#fff' , fontFamily: "'Playfair Display', 'Georgia', serif" }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }}>
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} style={{ position: 'relative' }}>
            <img
              ...
              style={{ width: '100%', height: '420px', objectFit: 'cover', borderRadius: '20px', display: 'block', boxShadow: '0 24px 60px rgba(0, 43, 92, 0.12)' }}
            />
            <motion.div initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }} style={{ position: 'absolute', bottom: '20px', right: '20px', zIndex: 2, background: '#fff', borderRadius: '16px', padding: '16px 24px', boxShadow: '0 8px 32px rgba(0,43,92,0.15)', fontFamily: "'DM Sans', sans-serif" }}>
              <div style={{ fontSize: '28px', fontWeight: 700, color: '#002b5c', fontFamily: "'Playfair Display', serif" }}>250+</div>
              <div style={{ fontSize: '13px', color: '#64748b', fontWeight: 500 }}>Students Enrolled</div>
            </motion.div>
          </motion.div>

          {/* Text Side */}
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <span style={{ display: 'inline-block', color: '#002b5c', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '16px' }}>Since May 2025</span>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 700, color: '#002b5c', lineHeight: 1.2, marginBottom: '20px' }}>
              Modern Education for{' '}<span style={{ color: '#d4af37' }}>Future Leaders</span>
            </h2>
            ...
            {['State Board curriculum with modern methodology', 'Safe, CCTV-monitored campus environment', 'Activity-based and experiential learning', 'Strong focus on character & communication', 'Caring and qualified teaching faculty'].map((point, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 + 0.2 }} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '14px', fontFamily: "'DM Sans', sans-serif", fontSize: '0.95rem', color: '#1a365d' }}>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.6 }} style={{ marginTop: '32px', display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <Link to="/about/vision-mission" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#002b5c', color: '#fff', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: '0.95rem', padding: '14px 28px', borderRadius: '50px', textDecoration: 'none', boxShadow: '0 6px 24px rgba(0, 43, 92, 0.25)' }}>Our Vision & Mission <ArrowRight size={18} /></Link>
              <Link to="/about/chairman" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', border: '2px solid #002b5c', color: '#002b5c', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: '0.95rem', padding: '14px 28px', borderRadius: '50px', textDecoration: 'none' }}>Meet Our Leaders</Link>
            </motion.div>
          </motion.div>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 0', background: '#fcfaf6' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginBottom: '52px' }}>
            <span style={{ display: 'inline-block', color: '#002b5c', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px' }}>Why Brindavan</span>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 700, color: '#002b5c', marginBottom: '12px' }}>Why Parents Choose Brindavan</h2>
            <p style={{ fontFamily: "'DM Sans', sans-serif", color: '#64748b', fontSize: '1.05rem' }}>Trusted by families across Palamaner for quality education</p>
          </motion.div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px' }}>
            {[
              ...
              <motion.div key={i} initial={{ y: 30, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} style={{ background: '#fff', borderRadius: '24px', padding: '32px 24px', border: '1px solid rgba(0, 43, 92, 0.06)', boxShadow: '0 15px 35px rgba(0, 43, 92, 0.05)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: 'linear-gradient(90deg, #002b5c, #d4af37)' }} />
                <div style={{ fontSize: '2.8rem', marginBottom: '16px' }}>{item.emoji}</div>
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.2rem', fontWeight: 700, color: '#002b5c', marginBottom: '8px' }}>{item.label}</h3>
                <p style={{ fontFamily: "'DM Sans', sans-serif", color: '#64748b', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '80px 0', background: '#fff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginBottom: '52px' }}>
            <span style={{ display: 'inline-block', color: '#002b5c', fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '12px' }}>Our DNA</span>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 700, color: '#002b5c' }}>Our Core Values</h2>
          </motion.div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
            {[
              ...
              <motion.div key={i} initial={{ y: 30, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} style={{ background: '#fff', borderRadius: '24px', padding: '32px 28px', border: '1px solid rgba(0, 43, 92, 0.08)', boxShadow: '0 15px 35px rgba(0, 43, 92, 0.05)', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                <div style={{ width: '56px', height: '56px', background: 'linear-gradient(135deg, rgba(0, 43, 92, 0.06), rgba(0, 43, 92, 0.02))', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px', border: '1.5px solid rgba(0, 43, 92, 0.1)' }}>
                  <value.icon size={26} style={{ color: '#002b5c' }} />
                </div>
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.2rem', fontWeight: 700, color: '#002b5c', marginBottom: '10px' }}>{value.title}</h3>
                <p style={{ fontFamily: "'DM Sans', sans-serif", color: '#64748b', fontSize: '0.92rem', lineHeight: 1.7, margin: 0 }}>{value.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* ── CTA ── */}
      <section style={{ padding: '80px 24px', background: 'linear-gradient(135deg, #002b5c 0%, #1a365d 50%, #102a45 100%)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '680px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/admissions" style={{ background: '#fff', color: '#002b5c', fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: '0.97rem', padding: '14px 32px', borderRadius: '50px', textDecoration: 'none', boxShadow: '0 8px 30px rgba(0,0,0,0.15)', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>Apply Now <ArrowRight size={17} /></Link>
